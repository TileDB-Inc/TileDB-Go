...
// for advanced usage
//
// Deprecated: Use SetDataBufferUnsafe, SetOffsetsBufferUnsafe and SetValidityBufferUnsafe instead.
func (q *Query) SetBufferVarNullableUnsafe(attribute string, offset unsafe.Pointer, offsetSize uint64, buffer unsafe.Pointer, bufferSize uint64, bufferValidity unsafe.Pointer, bufferValiditySize uint64) (*uint64, *uint64, *uint64, error) {
	...
	validitySizePtr, err := q.SetValidityBufferUnsafe(attribute, bufferValidity, bufferValiditySize)
	if err != nil {
		q.resetResultBufferPointers(attribute)
		return nil, nil, nil, err
	}

	return offsetSizePtr, bufferSizePtr, validitySizePtr, nil
}

// SetBufferVarNullable Sets the buffer for a variable sized nullable attribute/dimension to a query
// The buffer must be an initialized slice
//
// Deprecated: Use SetDataBuffer, SetOffsetsBuffer and SetValidityBuffer instead.
func (q *Query) SetBufferVarNullable(attributeOrDimension string, offset []uint64, buffer interface{}, bufferValidity []uint8) (*uint64, *uint64, *uint64, error) {
	...
	validitySizePtr, err := q.SetValidityBuffer(attributeOrDimension, bufferValidity)
	if err != nil {
		q.resetResultBufferPointers(attributeOrDimension)
		return nil, nil, nil, err
	}
...
// resetResultBufferPointers sets to nil all resultBufferElements for attribute.
// This is indented to be call if setting a resultBufferElement fails.
func (q *Query) resetResultBufferPointers(attribute string) {
	q.resultBufferElements[attribute] = [3]*uint64{}
}
